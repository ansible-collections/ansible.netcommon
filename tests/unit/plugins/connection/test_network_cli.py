...
# --- buffer drain tests (error path) ---


@pytest.mark.parametrize("ssh_type", ["paramiko", "libssh", "auto"])
def test_send_error_raises_with_correct_message(conn, ssh_type):
    """Error is raised after buffer drain with the correct error message."""
    conn.set_options(
        direct={
            "ssh_type": ssh_type,
    ...
    error_response = b"no vstack\n% Invalid input detected\ndevice#"

    if conn.ssh_type == "paramiko":
        mock__shell.recv.side_effect = [error_response, None]
    elif conn.ssh_type == "libssh":
        mock__shell.read_bulk_response.side_effect = [None, error_response, None]

    with pytest.raises(AnsibleConnectionFailure, match="Invalid input"):
        conn.send(b"no vstack")


@pytest.mark.parametrize("ssh_type", ["paramiko", "libssh", "auto"])
def test_send_error_drains_buffer_before_raising(conn, ssh_type):
    """When error+prompt is found, trailing data is consumed before raising."""
    conn.set_options(
        direct={
            "ssh_type": ssh_type,
            "terminal_stderr_re": [{"pattern": "Invalid input"}],
