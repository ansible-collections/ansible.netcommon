...
_CLI_ONLY_MODULES = frozenset(
    ["junos_netconf", "iosxr_netconf", "iosxr_config", "iosxr_command"]
)
...
class ActionModule(ActionBase):
    def run(self, tmp=None, task_vars=None):
        ...
        if play_context.connection == "local":
            ...
            self.provider = load_provider(
                module.get_provider_argspec(), self._task.args
            )
            if (
                self.provider.get("transport") == "netconf"
                and play_context.network_os in _NETCONF_SUPPORTED_PLATFORMS
                and self._task.action not in _CLI_ONLY_MODULES
            ):
                play_context.connection = "netconf"
                play_context.port = int(
                    self.provider["port"] or self._play_context.port or 830
                )
            elif self.provider.get("transport") in (
                "nxapi",
                "eapi",
            ) and play_context.network_os in ("nxos", "eos"):
                play_context.connection = "local"
                play_context.port = int(
                    self.provider["port"] or self._play_context.port or 22
                )
            else:
            ...
            play_context.remote_addr = (
                self.provider["host"] or self._play_context.remote_addr
            )
            play_context.remote_user = (
                self.provider["username"] or self._play_context.connection_user
            )
            play_context.password = (
                self.provider["password"] or self._play_context.password
            )
            play_context.private_key_file = (
                self.provider["ssh_keyfile"]
                or self._play_context.private_key_file
            )
            play_context.timeout = int(
                self.provider["timeout"] or C.PERSISTENT_COMMAND_TIMEOUT
            )
            ...
            if self._play_context.connection == "local":
                if (
                    self.provider.get("transport") == "nxapi"
                    and play_context.network_os == "nxos"
                ):
                    self._task.args[
                        "provider"
                    ] = _NxosActionModule.nxapi_implementation(
                        self.provider, self._play_context
                    )
                elif (
                    self.provider.get("transport") == "eapi"
                    and play_context.network_os == "eos"
                ):
                    self._task.args[
                        "provider"
                    ] = _EosActionModule.eapi_implementation(
                        self.provider, self._play_context
                    )
                else:
        ...
        if play_context.connection == "network_cli":
            ...
            out = conn.get_prompt()
            if (
                to_text(out, errors="surrogate_then_replace")
                .strip()
                .endswith(")#")
            ):
                display.vvvv(
                    "wrong context, sending exit to device",
        ...
        if not module:
            ...
            display.vvvv(
                "Caching network OS %s in facts" % play_context.network_os
            )
    ...
    def _start_connection(self, play_context):
        ...
        display.vvv(
            "using connection plugin %s (was local)" % play_context.connection,
            play_context.remote_addr,
        )
        connection = self._shared_loader_obj.connection_loader.get(
            "persistent", play_context, sys.stdin, task_uuid=self._task._uuid
        )

        connection.set_options(
            direct={"persistent_command_timeout": play_context.timeout}
        )
    ...
    def _get_implementation_module(self, network_os, platform_agnostic_module):
        implementation_module = (
            network_os + "_" + platform_agnostic_module.partition("_")[2]
        )
