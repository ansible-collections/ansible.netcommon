...
- The paramiko transport is provided because many distributions, in particular EL6
  and before do not support ControlPersist in their SSH implementations.
- This is needed on the Ansible control machine to be reasonably efficient with connections.
  Thus paramiko is faster for most users on these platforms. Users with ControlPersist
  capability can consider using -c ssh or configuring the transport in the configuration
  file.
- This plugin also borrows a lot of settings from the ssh plugin as they both cover
  the same protocol.
options:
  remote_addr:
    description:
  ...
  proxy_command:
    ...
    - Also this plugin will scan 'ssh_args', 'ssh_extra_args' and 'ssh_common_args'
      from the 'ssh' plugin settings for proxy information if set.
    env:
...
class Connection(ConnectionBase):
    ...
    transport = "ansible.netcommon.paramiko"
    _log_channel = None
    ...
    def _parse_proxy_command(self, port=22):
        ...
        if proxy_command:
            ...
            try:
                ...
                display.vvv(
                    "CONFIGURE PROXY COMMAND FOR CONNECTION: %s"
                    % proxy_command,
