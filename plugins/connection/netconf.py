...
options:
  ...
  timeout:
    ...
    - Sets the connection time, in seconds, for communicating with the remote device.  This
      timeout is used as the default timeout value when awaiting a response after
      issuing a call to a RPC.  If the RPC does not return in timeout seconds, an
      error is generated.
    default: 120
  look_for_keys:
    default: true
...
class Connection(NetworkConnectionBase):
    ...
    def _connect(self):
        ...
        device_params = {
            "name": NETWORK_OS_DEVICE_PARAM_MAP.get(self._network_os)
            or self._network_os
        }
